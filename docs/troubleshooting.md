...
- **対策**: CRITICAL/MAJORの指摘は投稿前にファイル全文で再検証されます（[verify_high_severity_findings()](../src/reviewer.py)）。全文を見ても確認できない指摘は自動的に破棄されるため、このパターンの誤検知は大幅に減っているはずです。それでも発生した場合はコード修正せず、PRスレッドに理由を返信して解決（resolve）する。
